...
class ProcessingPipeline:
    ...
    def __init__(self):
        ...
        self.whisper = WhisperGPU(model_size="base")
        self.ocr = OCRService()
    ...
    def _process(self, job_id: str, url: str, schema: Dict, prompt: str):
        ...
        try:
            # Step 1: Download
            self.job_store.update_job(job_id, {'status': 'downloading', 'progress': {'step': 1, 'total': 3}})
            video_path = self.downloader.download(url, job_id)
            
            # Step 2: Transcribe
            self.job_store.update_job(job_id, {'status': 'transcribing', 'progress': {'step': 2, 'total': 3}})
            transcription = self.whisper.transcribe(video_path)
            
            # Step 3: OCR
            self.job_store.update_job(job_id, {'status': 'extracting_text', 'progress': {'step': 3, 'total': 3}})
